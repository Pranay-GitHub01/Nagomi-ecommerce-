...
const WallArtCard: React.FC<WallArtCardProps> = ({ wallArt }) => {
  ...
  useEffect(() => {
    const checkStatus = async () => {
      ...
      const status = user
        ? await checkWishlistStatus(wallArtId)
        : isInWishlist(wallArtId);
      setIsWishlisted(status);
    };
    checkStatus();
  }, [user, wallArt._id, wallArt.id, checkWishlistStatus, isInWishlist]);
  ...
  const displayPrice = formatPrice(currentVariant?.sellingPrice?.[0] || wallArt.price);
  const displayOriginalPrice = formatPrice(currentVariant?.mrp?.[0] || wallArt.originalPrice);
  ...
  return (
    <>
      ...
      >
        <Link to={`/wallart/${wallArt._id || wallArt.id}`} className="block flex flex-col h-full">
          <div className="relative overflow-hidden">
            {/* ### REVERTED TO YOUR FIXED URL STRUCTURE ### */}
            <motion.img
              src={`https://server-hule.onrender.com/images/wall_art/WA_${wallArt.skuId.slice(-3)}/${parseInt(wallArt.skuId.slice(-3))}.1_${wallArt.colors?.[0]?.[0] || 'G'}_WA.png`}
              alt={wallArt.name}
              className="w-full h-64 object-cover group-hover:scale-110 transition-transform duration-500"
              whileHover={{ scale: 1.12 }}
            />
          ...
          <div className="p-4 flex flex-col flex-grow">
            ...
            <p className="font-bold italic text-[#545454] mb-2">
              {displayOriginalPrice ? (
                <>
                  <span className="font-bold italic text-gray-400 line-through">
                    {displayOriginalPrice}
                  </span>{" "}
                  {displayPrice}
                </>
            ...
            {wallArt.variants && wallArt.variants.length > 1 && (
              <div className="mt-auto pt-2 flex gap-2 justify-center">
                {wallArt.variants.map((variant, index) => (
                  <button
