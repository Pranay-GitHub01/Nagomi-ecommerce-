...
//   title: string;
//   sectionKey: keyof WallArtPageState["openSections"];
//   children: React.ReactNode;
//   openSections: WallArtPageState["openSections"];
//   setOpenSections: React.Dispatch<
//     React.SetStateAction<WallArtPageState["openSections"]>
//   >;
// }> = ({ title, sectionKey, children, openSections, setOpenSections }) => {
//   return (
...
//         <ChevronDown
//           className={`w-5 h-5 text-[#1428a0] transition-transform ${
...
//     "popularity" | "price-low" | "price-high" | "newest" | "alphabetical" | ""
//   >("popularity");
...
//     (key: "colors" | "materials", value: string) => {
//       /* ... keep original ... */
//       setFilters((prev) => {
...
//         const itemsWithDisplaySrc = filteredList.map(
//           (item): ProductWithDisplaySrc => {
...
//               displaySrc = item.images[0].startsWith("/")
//                 ? item.images[0]
//                 : `/${item.images[0]}`;
//             }
...
//         setWallArtItems(
//           itemsWithDisplaySrc.length > 0 ? itemsWithDisplaySrc : []
//         );
//         setIsLoading(false);
//       })
//       .catch((error) => {
...
//       new Set(wallArtItems.flatMap((item) => item.colors || []).filter(Boolean))
//     );
//     const uniqueMaterials = Array.from(
//       new Set(
//         wallArtItems.flatMap((item) => item.materials || []).filter(Boolean)
//       )
//     );
...
//     /* ... Keep original filtering/sorting logic ... */
...
//       const lowerCaseSelectedColors = filters.colors.map((c) =>
//         c.toLowerCase()
//       );
//       workingList = workingList.filter(
//         (item) =>
...
//       const lowerCaseSelectedMaterials = filters.materials.map((m) =>
//         m.toLowerCase()
//       );
//       workingList = workingList.filter(
//         (item) =>
...
//             lowerCaseSelectedMaterials.includes(
//               (itemMaterial || "").toLowerCase()
//             )
//           )
//       );
//     }
...
//             (b.skuId || "").localeCompare(a.skuId || "")
//         );
//         break; // Use createdAt if available
//       case "alphabetical":
...
//         ); // Sort by bestseller status first
//     }
...
//     isMobile: boolean /* ... Keep original ... */
//   ) => (
//     <aside className="w-full">
//       {/* Desktop Header */}
//       <div className="hidden lg:flex justify-between items-center mb-4">
//         <h2 className="text-xl font-bold text-[#172b9b] font-seasons">
//           Filters
//         </h2>
//         {isAnyFilterActive && (
//           <button
//             onClick={handleClearAllFilters}
//             className="text-sm text-blue-600 hover:underline font-lora font-semibold"
//           >
//             Clear All
//           </button>
//         )}
...
//         <meta
//           name="description"
//           content="Browse our extensive collection of premium wall art..."
//         />
//       </Helmet>
...
//               </h1>
//               {/* Subtitle */}
//               <p className="text-lg md:text-xl text-gray-600 max-w-4xl mx-auto italic font-semibold font-lora">
//                 {" "}
//                 {/* Adjusted font weight */}
//                 Transform every wall into a masterpiece with our aesthetically
...
//                 <p className="text-sm text-[#1428a0] font-lora">
//                   {wallArtToShow.length} Results found
//                 </p>{" "}
//                 {/* Adjusted text */}
...
//                   <label
//                     htmlFor="sort-by"
//                     className="text-sm font-bold text-[#172b9b] font-lora"
//                   >
//                     Sort by:
//                   </label>
//                   <select
...
//                     style={{
//                       background: `url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%236b7280' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e") no-repeat right 0.75rem center/1em`,
...
//                     ></path>
//                   </svg>
//                   <h3 className="text-xl font-semibold text-gray-700 font-lora">
//                     No Product Found
//                   </h3>
...
//                     <button
//                       onClick={handleClearAllFilters}
//                       className="mt-6 px-5 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold shadow-md hover:shadow-lg transition-all duration-150"
//                     >
//                       Clear Filters
//                     </button>
//                   )}
...
//                               className={`px-3 py-1.5 rounded-md border bg-white text-gray-600 border-gray-300 hover:bg-gray-50 transition-colors hidden sm:inline-block`}
...
//                               <span
//                                 key="start-ellipsis"
//                                 className="px-2 text-gray-400 hidden sm:inline-block"
//                               >
//                                 ...
//                               </span>
//                             );
...
//                               className={`px-3 py-1.5 rounded-md border transition-colors ${
//                                 currentPage === page
//                                   ? "bg-[#172b9b] text-white border-[#172b9b] font-semibold z-10"
//                                   : "bg-white text-gray-600 border-gray-300 hover:bg-gray-50"
//                               }`}
//                               aria-current={
...
//             className="flex items-center justify-center gap-2 py-2 text-sm text-gray-700 font-semibold border-r border-gray-200"
//           >
...
//             className="flex items-center justify-center gap-2 py-2 text-sm text-gray-700 font-semibold"
//           >
//             <ListFilter className="w-5 h-5" />
//             <span>Filters</span>
//             {isAnyFilterActive && (
//               <span className="w-1.5 h-1.5 bg-blue-500 rounded-full inline-block ml-1"></span>
//             )}
//           </button>
//         </div>
//       </div>
...
//                     }}
//                     className={`w-full text-left py-3 px-2 text-sm rounded-md transition-colors ${
//                       sortBy === opt.value
//                         ? "text-[#172b9b] font-semibold bg-blue-50"
...
//                 <h2 className="text-lg font-semibold text-[#172b9b] font-seasons">
//                   Filter Options
//                 </h2>
//                 {isAnyFilterActive && (
//                   <button
...
//                     }}
//                     className="text-xs text-gray-500 hover:text-red-600 font-lora"
//                   >
//                     {" "}
//                     Clear All{" "}
//                   </button>
...
//                   <label
//                     htmlFor="mobile-sidebar-search"
//                     className="block text-sm font-medium text-gray-600 mb-2 font-lora"
//                   >
//                     Search Wall Art
//                   </label>
//                   <input
...
const LuxePage: React.FC = () => {
  const [wallArtItems, setWallArtItems] = useState<ProductWithDisplaySrc[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [currentPage, setCurrentPage] = useState(1);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/products`)
      ...
      .then((allProducts) => {
        ...
        // Filter for Luxe categories
        const filteredList = allProducts.filter(
          (item: any): item is Product => {
            const category = (item?.category || "").toString().toLowerCase();
            return category === "luxe" || category === "luxe collection";
          }
        );
  ...
  const totalPages = Math.ceil(wallArtItems.length / PER_PAGE);
  const paginatedItems = useMemo(() => {
    return wallArtItems.slice(
      (currentPage - 1) * PER_PAGE,
      currentPage * PER_PAGE
    );
  }, [wallArtItems, currentPage]);
  ...
  return (
    <>
      <Helmet>
        <title>Luxe Collection | Nagomi</title>
        <meta
          name="description"
          content="Experience the art of luxury - embroidered, embellished, and exquisitely designed."
        />
      </Helmet>
      ...
      <div className="min-h-screen bg-white">
        ...
        {/* --- SIMPLE TEXT HEADER SECTION (Matches your image) --- */}
        <div className="pb-8 px-4 w-full text-center bg-white">
            
            {/* Breadcrumb: Home > Luxe Collection */}
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-4 text-left">
                <nav className="text-sm font-lora text-blue-800">
                    <Link to="/" className="hover:underline">Home</Link>
                    <span className="mx-2">&gt;</span>
                    <span className="font-semibold">Luxe Collection</span>
                </nav>
            </div>
            ...
            <motion.h1 
                ...
                animate={{ opacity: 1, y: 0 }}
                className="text-4xl md:text-5xl font-serif text-[#172b9b] mb-3 font-normal"
            >
                Luxe Collection
            </motion.h1>
            ...
            <motion.p 
                ...
                className="text-gray-600 font-serif italic text-lg md:text-xl"
            >
                Experience the art of luxury - embroidered, embellished, and exquisitely designed
            </motion.p>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {isLoading ? (
            ...
            <div className="mt-8 text-center py-12 bg-white rounded-xl shadow-lg border border-blue-100">
              ...
              <p className="text-gray-500 mt-2 font-lora">
                Check back later for new exclusive designs.
              </p>
            </div>
          ) : (
            <>
              {/* Product Grid */}
              <div className="grid gap-4 md:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                {paginatedItems.map((productItem, index) => (
                  <motion.div
                    key={productItem._id || productItem.id}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
              ...
              {/* Pagination */}
              {totalPages > 1 && (
                <div className="flex justify-center items-center gap-1.5 mt-12 mb-20 text-sm font-lora">
                  <button
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                     <button
                        ...
                        className={`px-4 py-2 rounded-md border transition-colors ${
                          currentPage === page
                            ? "bg-[#172b9b] text-white border-[#172b9b] font-semibold"
                            : "bg-white text-gray-600 border-gray-300 hover:bg-gray-50"
                        }`}
                      >
                        {page}
