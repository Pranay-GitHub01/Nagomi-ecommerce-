...
//   if (!open) return null;
//   return (
//     <div className="fixed inset-0 z-50 flex items-center justify-center w-1/2 bg-black bg-opacity-40">
...
//               {/* Product Title */}
//               <h1 className="text-[40px] font-bold text-[#172b9b] font-seasons m-0">
//                 {product.name}
//               </h1>

//               {/* Ratings */}
//               <div className="flex items-center gap-2 m-0" style={{ marginTop: '-4px' }}>
...
//                    className="w-1/2 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#172b9b] focus:border-[#172b9b]"
//                  >
...
import { FaWhatsapp } from 'react-icons/fa';
import { usePincodeCheck } from "../hooks/usePincodeCheck"; // Added Import
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
        <div className="bg-white rounded-lg shadow-lg p-6 max-w-lg w-full relative animate-fade-in-scale">
          <button onClick={onClose} className="absolute top-2 right-2 text-gray-400 hover:text-gray-700 text-2xl leading-none">&times;</button>
          <h2 className="text-xl font-semibold mb-3 text-[#172b9b] font-seasons">Wallpaper Material Guide</h2>
          <p className="mb-4 text-gray-600 font-lora text-sm">Choose the best material for your needs:</p>
          <ul className="space-y-3 text-gray-700 font-lora text-sm">
            <li><b>Non-woven Smooth (₹99/sq ft):</b> Eco-friendly, breathable, easy to install & remove (dry strippable). Matte finish. Great for most rooms.</li>
            <li><b>Vinyl Matte (₹119/sq ft):</b> Durable and washable surface. Moisture-resistant, suitable for kitchens/bathrooms. Matte finish.</li>
            <li><b>Glitter Finish (₹119/sq ft):</b> Adds subtle sparkle. Durable vinyl base. Good for feature walls.</li>
            <li><b>Canvas Texture (₹149/sq ft):</b> Mimics painted canvas. Adds depth and a luxurious, artistic touch.</li>
...
const ProductDetail: React.FC = () => {
  ...
  // Updated: Defaults to false so user checks pincode first
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
              ...
              >
                 ...
                 {imageList.length > 1 && (
                   <>
                     <button
                       onClick={() => setSelectedImageIndex(prev => prev === 0 ? imageList.length - 1 : prev - 1)}
                       className="absolute left-3 top-1/2 transform -translate-y-1/2 bg-white/60 backdrop-blur-sm p-2 rounded-full shadow hover:bg-white focus:outline-none focus:ring-2 focus:ring-[#172b9b] transition-all z-20"
                       aria-label="Previous image"
                     >
                       <ChevronLeft className="w-5 h-5 text-gray-800" />
                     </button>
                     <button
                       onClick={() => setSelectedImageIndex(prev => prev === imageList.length - 1 ? 0 : prev + 1)}
                       className="absolute right-3 top-1/2 transform -translate-y-1/2 bg-white/60 backdrop-blur-sm p-2 rounded-full shadow hover:bg-white focus:outline-none focus:ring-2 focus:ring-[#172b9b] transition-all z-20"
                       aria-label="Next image"
                     >
                       <ChevronRight className="w-5 h-5 text-gray-800" />
                     </button>
            ...
            <div className="space-y-6">
              ...
              {/* --- NEW PINCODE CHECK SECTION (Matched with WallRollDetail) --- */}
              <div className="mb-6">
                 ...
                 {/* Fallback delivery message if pin checked ok */}
                 {!pinLoading && pinCode.length === 6 && city && ( 
                     <div className="text-xs text-green-700 mt-1 font-lora"> </div> 
                 )}
              </div>

              {/* --- UPDATED INSTALLATION OPTION (Matched with WallRollDetail logic) --- */}
              <div className="flex items-center gap-2 mt-2 mb-6">
                <input
                  type="checkbox"
                  id="install-checkbox"
                  checked={includeInstallation}
                  disabled={!isSupported} // Disabled if not supported by pincode check
                  onChange={(e) => setIncludeInstallation(e.target.checked)}
                  className="w-4 h-4 text-[#172b9b] border-gray-300 rounded disabled:opacity-40 disabled:cursor-not-allowed"
                />
                <label htmlFor="install-checkbox" className="text-sm font-medium text-gray-700 font-lora">
                  Include installation (₹9/square feet)
                  {!isSupported && (
                    <span className="text-red-600 ml-1 text-xs">(Enter Pin Code)</span>
                  )}
                </label>
              </div>
